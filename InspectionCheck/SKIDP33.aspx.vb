        ...
        If Query = "" Then
            If Flag.FlagMode = False Then
                If Cb_Week.SelectedValue = "All" Then
                    SQL.ExecQuery("SELECT Waktu.ID, SKID.InspectionCheck, SKID.Standard, SKID.Week, Waktu.Result, Waktu.Tanggal, Waktu.Remark, Waktu.PIC, Waktu.Bulan FROM SKID INNER JOIN Waktu ON SKID.ID = Waktu.Idc WHERE Waktu.Bulan = '" & Namabulan & "' AND Waktu.Tahun = '" & tahun & "' AND SKID.SKID LIKE '%P33%' AND (Result IS NULL OR Result = '' OR Tanggal IS NULL OR Tanggal = '' OR Remark IS NULL OR Remark = '' OR PIC IS NULL OR PIC = '');")

                Else
                    SQL.ExecQuery("SELECT Waktu.ID, SKID.InspectionCheck, SKID.Standard, SKID.Week, Waktu.Result, Waktu.Tanggal, Waktu.Remark, Waktu.PIC, Waktu.Bulan FROM SKID INNER JOIN Waktu ON SKID.ID = Waktu.Idc WHERE Waktu.Bulan = '" & Namabulan & "' AND Waktu.Tahun = '" & tahun & "' AND SKID.Week = '" & Cb_Week.SelectedValue & "' AND SKID.SKID LIKE '%P33%' AND (Result IS NULL OR Result = '' OR Tanggal IS NULL OR Tanggal = '' OR Remark IS NULL OR Remark = '' OR PIC IS NULL OR PIC = '');")
                End If
            ElseIf Flag.FlagMode = True Then
                If Cb_Week.SelectedValue = "All" Then
                    SQL.ExecQuery("SELECT Waktu.ID, SKID.InspectionCheck, SKID.Standard, SKID.Week, Waktu.Result, Waktu.Tanggal, Waktu.Remark, Waktu.PIC, Waktu.Bulan FROM SKID INNER JOIN Waktu ON SKID.ID = Waktu.Idc WHERE Waktu.Bulan = '" & Namabulan & "' AND Waktu.Tahun = '" & tahun & "' AND SKID.SKID LIKE '%P33%';")

                Else
                    SQL.ExecQuery("SELECT Waktu.ID, SKID.InspectionCheck, SKID.Standard, SKID.Week, Waktu.Result, Waktu.Tanggal, Waktu.Remark, Waktu.PIC, Waktu.Bulan FROM SKID INNER JOIN Waktu ON SKID.ID = Waktu.Idc WHERE Waktu.Bulan = '" & Namabulan & "' AND Waktu.Tahun = '" & tahun & "' AND SKID.Week = '" & Cb_Week.SelectedValue & "' AND SKID.SKID LIKE '%P33%';")
                End If
            End If
        Else
            SQL.ExecQuery(Query)
